...
// Standby/wake subscriptions are per-user in this demo (the real system also
// scopes them per-device; simplified here since device management is a
// separate concern from the comms features this server showcases).
async function armChannel({ userId, channelId }) {
  await db.query(
    `INSERT INTO ptt_channel_subscriptions (user_id, channel_id, wake_enabled)
     VALUES ($1, $2, TRUE)
...
async function disarmChannel({ userId, channelId }) {
  await db.query(
    `DELETE FROM ptt_channel_subscriptions WHERE user_id = $1 AND channel_id = $2`,
    [userId, channelId],
  );
  return true;
}

// A muted channel still counts as armed (the subscription row stays put —
// arm/disarm reflect "am I subscribed at all", mute reflects "should this
// device be interrupted for it"). No-op if not subscribed yet.
async function setChannelMuted({ userId, channelId, muted }) {
  const { rowCount } = await db.query(
    `UPDATE ptt_channel_subscriptions
     SET muted = $3, updated_at = NOW()
     WHERE user_id = $1 AND channel_id = $2`,
    [userId, channelId, !!muted],
  );
  return rowCount > 0;
}

async function syncSubscriptions({ userId, channels }) {
  const requested = Array.isArray(channels) ? channels.slice(0, 100) : [];
  const normalized = new Map();
  for (const item of requested) {
    if (typeof item?.channelId !== 'string' || !item.channelId) continue;
    normalized.set(item.channelId, item.wakeEnabled !== false);
  }

  const accepted = [];
  const rejected = [];
  for (const [channelId, wakeEnabled] of normalized) {
    if (await resolveAccess(userId, channelId)) accepted.push({ channelId, wakeEnabled });
    else rejected.push(channelId);
  }

  await db.transaction(async client => {
    await client.query(
      'DELETE FROM ptt_channel_subscriptions WHERE user_id = $1',
      [userId],
    );
    for (const item of accepted) {
      // ON CONFLICT matches armChannel's upsert above: a full resync can
      // legitimately race a single-channel arm and a plain INSERT would
      // crash the handler with a duplicate-key violation instead of just
      // converging on the same row.
      await client.query(
        `INSERT INTO ptt_channel_subscriptions (user_id, channel_id, wake_enabled)
         VALUES ($1, $2, $3)
         ON CONFLICT (user_id, channel_id) DO UPDATE
           SET wake_enabled = EXCLUDED.wake_enabled, updated_at = NOW()`,
        [userId, item.channelId, item.wakeEnabled],
      );
    }
  });
  return { acceptedChannelIds: accepted.map(item => item.channelId), rejectedChannelIds: rejected };
}

async function deleteUserChannelSubscription(channelId, userId) {
  await db.query(
    'DELETE FROM ptt_channel_subscriptions WHERE channel_id = $1 AND user_id = $2',
    [channelId, userId],
  );
}

async function deleteChannelSubscriptions(channelId) {
  await db.query(
    'DELETE FROM ptt_channel_subscriptions WHERE channel_id = $1',
    [channelId],
  );
}

async function deleteUserWorkspaceSubscriptions(workspaceId, userId) {
  await db.query(
    `DELETE FROM ptt_channel_subscriptions subscriptions
     USING ptt_channels channels
     WHERE subscriptions.channel_id = channels.id
       AND channels.workspace_id = $1
...
module.exports = {
  ...
  setChannelMuted,
  syncSubscriptions,
  deleteUserChannelSubscription,
  deleteChannelSubscriptions,
  deleteUserWorkspaceSubscriptions,
